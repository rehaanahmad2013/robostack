...
class BCFrankaAgent(BCAgent):
	def __init__(self,
		...
		if self.from_vision == False:
			self.feature_dim = franka_dim
			franka_dim = 0
	...
	def _split_obs_state(self, batch):
		...
		obs_state, next_obs_state = obs['state'], next_obs['state']
		obs_img, next_obs_img = None, None
		if self.from_vision:
			obs_img, next_obs_img = obs['images'], next_obs['images']
		else:
			obs_img = obs_state
	...
	# just state version of the function
	def compute_log_prob_state(self, obs_state, actions):
		''' useful when only the loss is needed, for example validation.'''
		metrics = dict()
