...
def make(name, action_repeat, reward_type='sparse', height=84, width=84, num_frames=1):
	...
	if 'franka' in name:
		...
		if 'cubepickup' in name:
			...
			backward_demos = dict(np.load(backward_path))
		elif 'bcfrankatest' in name:
			DoF = 6
			episodic_path_length = 120
			forward_path = f'/iris/u/rehaan/BCARL/demos/2dchoice.npz'
			forward_demos = dict(np.load(forward_path))
		...
		train_env = FrankaObsActionDTypeWrapper(train_env, np.float32, {'state': np.float32, 'images': np.float32})
