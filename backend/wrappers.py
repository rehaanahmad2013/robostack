...
class FrankaDMEnvFromGymWrapper(DMEnvFromGym):
    def __init__(self, gym_env: gym.Env, state='ee', height=100, width=100):
        self.gym_env = gym_env
        # Convert gym action and observation spaces to dm_env specs.
        # obs_box = self.gym_env.observation_space
        self._state = state
        if self._state == 'ee':
            ...
            state_spec = curobsspace

        obs_box = Dict({'images': Box(np.full((3, height, width), 0),
                                 np.full((3, height, width), 255)),
                        'state': state_spec})

        # obs_box = Dict({'state': state_spec})
        self._observation_spec = space2spec(obs_box,
                                            name='observation')

        curactionspace = Box(
            np.array([-1] * (7)), # dx_low, dy_low, dz_low, dgripper_low
            np.array([ 1] * (7)), # dx_high, dy_high, dz_high, dgripper_high
        )

        self._action_spec = space2spec(curactionspace, name='action')
        self._reset_next_step = False

    def _franka_transform_observation(self, obs):
        fp_image = obs['third_person_img_obs'].transpose(2, 0, 1)
        # tp_image = obs['third_person_img_obs'].transpose(2, 0, 1)
        # state = obs['lowdim_qpos'] if self._state == 'qpos' else obs['lowdim_ee']

        new_obs = {'images': fp_image,
                   'state': obs['lowdim_obs']}
        # new_obs = {'state': obs['lowdim_obs']}
        return new_obs
