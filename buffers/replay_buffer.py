    ...
    def add_offline_data_franka(self, demos):
        ...
        for idx in range(acts.shape[0]):
            if idx == 0 or term[idx-1][0]:
                print("-----OBS IDX-------")
                print(obs[idx])
                obs[idx]['state'] = obs[idx]['state'].astype(np.float32)
                obs[idx]['images'] = obs[idx]['images'].astype(np.float32)
            ...
            next_obs[idx]['state'] = next_obs[idx]['state'].astype(np.float32)
            next_obs[idx]['images'] = next_obs[idx]['images'].astype(np.float32)
        ...
        if isinstance(spec, dict):
            for key in spec.keys():
                ...
                if spec[key].shape != val[key].shape or spec[key].dtype != val[key].dtype:
                    print("spec shape: " + str(spec[key].shape) + " val shape: " + str(val[key].shape) + " spec dtype: " + str(spec[key].dtype) + " val dtype: " +  str(val[key].dtype))
                    return False
            return True

        print("spec shape: " + str(spec.shape) + " val shape: " + str(val.shape) + " spec dtype: " + str(spec.dtype) + " val dtype: " +  str(val.dtype))
